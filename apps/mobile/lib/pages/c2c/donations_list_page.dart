          ...
          Expanded(
            child: donations.isLoading
                ...
                : donations.donations.isEmpty
                    ? _EmptyDonations(
                        l10n: l10n,
                        filtered: _selectedCommune != null,
                        onReset: () => _onCommuneChanged(null),
                      )
                    : RefreshIndicator(
                        onRefresh: () =>
                            donations.loadDonations(neighborhood: _selectedCommune),
                        child: ListView.builder(
                          padding: const EdgeInsets.all(16),
                          itemCount: donations.donations.length + (_showBanner ? 1 : 0),
                          itemBuilder: (context, index) {
                            if (_showBanner && index == 0) {
                              return ExplainerBanner(
                                emoji: '🔒',
                                title: 'Vos échanges restent privés',
                                body:
                                    'Seul le quartier est visible — jamais votre adresse '
                                    'exacte ni votre numéro de téléphone.',
                                onDismiss: () => setState(() => _showBanner = false),
                              );
                            }
                            final di = index - (_showBanner ? 1 : 0);
                            final donation =
                                donations.donations[di] as Map<String, dynamic>;
                            return FadeSlideIn(index: di, child: _DonationCard(donation: donation));
                          },
                        ),
                      ),
          ),
...
class _EmptyDonations extends StatelessWidget {
  ...
  const _EmptyDonations({required this.l10n, required this.filtered, required this.onReset});
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            filtered ? 'Aucun don dans ce quartier' : l10n.noOffersAvailable,
            style: TextStyle(color: Colors.grey.shade500),
          ),
          if (filtered) ...[
            const SizedBox(height: 12),
            TextButton(onPressed: onReset, child: const Text('Voir tous les quartiers')),
          ],
        ],
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(donation['description'] ?? '', maxLines: 3, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 8),
            Row(
              children: [
                Icon(Icons.location_on, size: 16, color: Colors.grey.shade600),
                const SizedBox(width: 4),
                Text(donation['neighborhood'] ?? '', style: TextStyle(color: Colors.grey.shade600)),
                const Spacer(),
                ...
                IconButton(
                  ...
                  onPressed: () {
                    SharePlus.instance.share(ShareParams(text: l10n.shareSuccess));
                  },
                ),
              ],
            ),
                ...
                onPressed: isReserved
                    ...
                    : () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                              content: Text('Messagerie en cours de développement (MVP)')),
                        );
                      },
